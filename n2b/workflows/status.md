...
| 5 | Between stages | `pipeline_status: paused` AND `last_completed_stage < 4` | "**Stage {N+1}: {Name}** — {description}" + "`/n2b:s{N+1}-{slug}`" where N = `last_completed_stage` — render the Next-Stage Lookup row's note verbatim when present (the Stage 3 row carries the batch hint, so a user landing after Stage 2 learns about `--continue` here as well as in condition 3) |
